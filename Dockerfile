...
RUN dotnet build "./src/Wiz.Chapter4.API/Wiz.Chapter4.API.csproj" -c Debug
CMD dotnet run --project src/Wiz.Chapter4.API/Wiz.Chapter4.API.csproj
...
COPY . .
RUN dotnet restore . -s $nuget_endpoint -s "https://api.nuget.org/v3/index.json"
RUN dotnet build "./src/Wiz.Chapter4.API/Wiz.Chapter4.API.csproj" -c Release

FROM build AS publish
RUN dotnet publish "./src/Wiz.Chapter4.API/Wiz.Chapter4.API.csproj" -c Release -o /publish

FROM mcr.microsoft.com/dotnet/core/aspnet:3.1-buster-slim AS production
ENV ASPNETCORE_ENVIRONMENT=Production
WORKDIR /app
COPY --from=publish /publish .
ENTRYPOINT ["dotnet", "Wiz.Chapter4.API.dll"]
EXPOSE 80